      ...
      {filteredCampaigns.length === 0 ? (
        ...
        (() => {
          ...
          const totalItems = Math.max(
            screenConfig.lg.columns * screenConfig.lg.rows, // Max items needed for large screens
            Math.min(
              filteredCampaigns.length,
              screenConfig.default.columns * screenConfig.default.rows,
            ), // Ensure no extra rows
          );

          const campaignsToShow = filteredCampaigns.slice(0, totalItems);

          return (
            <div
              className="
          grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5
          gap-4 px-4 sm:px-0 relative"
            >
