...
interface KPI {
  ...
  performance_vs_target?: {  // Change from null to optional
...
const KPIDashboardModal: React.FC<KPIDashboardModalProps> = ({
  ...
  const trendData = kpis.filter(kpi => kpi.trend && Object.keys(kpi.trend).length > 0)
    .slice(0, 3)
    .flatMap(kpi => 
  ...
  const radarData = primaryKpis.map(kpi => ({
    ...
    fullMark: Math.max(kpi.target_value * 1.5, (kpi.latest_value?.value || 0) * 1.5)
  }));
    ...
    >
      <div className="space-y-6">
                ...
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-2">
                      <div className={`p-2 rounded-full ${getKpiTypeColor(kpi.kpi_type)}`}>
                        {getKpiIcon(kpi.kpi_type)}
                      </div>
                      <div>
                        <p className="font-medium">{kpi.name}</p>
                        <p className="text-xs text-muted-foreground capitalize">{kpi.kpi_type}</p>
                      </div>
                    </div>
                    <Badge variant={isOnTarget ? "default" : isBehind ? "destructive" : "secondary"}>
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="font-medium">
                          {kpi.latest_value?.value?.toLocaleString() || 'N/A'} {kpi.unit}
                        </span>
                      </div>
                    ...
                    <div className="flex justify-between text-sm">
                      <span>Variance</span>
                      <span className={`font-medium ${kpi.performance_vs_target?.difference && kpi.performance_vs_target.difference >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {kpi.performance_vs_target?.difference ? (
                          <>
                            {kpi.performance_vs_target.difference >= 0 ? '+' : ''}
                            {kpi.performance_vs_target.difference.toLocaleString()} {kpi.unit}
                          </>
                        ) : 'N/A'}
                      </span>
                    </div>
                  ...
                  <div className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <RechartsBarChart data={performanceData}>
                        ...
                        <Tooltip />
                        <Legend />
                        <Bar dataKey="current" fill="#0088FE" name="Current Value" />
                          ...
                          <div className="flex items-center space-x-4">
                            ...
                            <div>
                              ...
                              <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                                <span className="capitalize">{kpi.kpi_type}</span>
                                <span>•</span>
                          ...
                          <div className="flex items-center space-x-4">
                            <div className="text-right">
                              ...
                              <div className="text-sm text-muted-foreground">
                                Target: {kpi.target_value?.toLocaleString() || 'N/A'}
                              </div>
                            </div>
                            ...
                            <div className="flex flex-col items-center">
                              ...
                              <div className={`text-xs mt-1 ${isOnTarget ? 'text-green-600' : isBehind ? 'text-red-600' : 'text-yellow-600'}`}>
                                {achievement.toFixed(1)}%
                              </div>
                            </div>
                          </div>
                ...
                {loading ? (
                  ...
                  <div className="space-y-6">
                    {kpis.map((kpi) => {
                      ...
                      const previousValue = trend[trendDates[trendDates.length - 2]];
                      const growth = previousValue ? ((latestValue || 0) - previousValue) / previousValue * 100 : 0;
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
                              ...
                              <div className="flex items-center space-x-4 mt-4 md:mt-0">
                                ...
                                <div className="text-center">
                                  <div className={`text-lg font-bold ${achievement >= 90 ? 'text-green-600' : achievement < 70 ? 'text-red-600' : 'text-yellow-600'}`}>
                                    {achievement.toFixed(1)}%
                                  </div>
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="space-y-1">
                                  <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium">{kpi.target_value?.toLocaleString() || 'N/A'}</span>
                                  </div>
                                  <div className="flex justify-between text-sm">
                                    <span>Current Value</span>
                                    <span className="font-medium">{latestValue?.toLocaleString() || 'N/A'}</span>
                                  </div>
                                  <div className="flex justify-between text-sm">
                                    <span>Variance</span>
                                    <span className={`font-medium ${kpi.performance_vs_target?.difference && kpi.performance_vs_target.difference >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="space-y-1">
                                  <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium">{trendDates.length}</span>
                                  </div>
                                  <div className="flex justify-between text-sm">
                                    <span>Latest Update</span>
                                    <span className="font-medium">
                                      {kpi.latest_value?.period_date ? formatDate(kpi.latest_value.period_date) : 'N/A'}
                                    </span>
                                  </div>
                                  <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium">
                                      {trendDates.length > 0 
                                        ? (Object.values(trend).reduce((a, b) => a + b, 0) / trendDates.length).toLocaleString() 
                                        : 'N/A'}
                                    </span>
                                  </div>
                                </div>
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="text-xs text-muted-foreground">
                                    {achievement >= 90 ? 'On Target' : achievement < 70 ? 'Needs Attention' : 'Moderate Progress'}
                                  </div>
                                </div>
                              </div>
                            </div>
        ...
        <div className="flex justify-between items-center pt-4 border-t">
          <div className="text-sm text-muted-foreground">
            {kpis.length} KPIs tracked • Last updated {formatDate(new Date().toISOString())}
          </div>
