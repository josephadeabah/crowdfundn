...
export function ConversationChat({
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/deal_room_conversations/${conversationId}/messages`,
        {
          headers: {
      ...
      showAlertMessage('Error', 'Failed to load messages', 'error');
    } finally {
      setIsLoading(false);
    }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      showAlertMessage('Error', 'Failed to send message', 'error');
    } finally {
      setIsSending(false);
    }
  };
  ...
  const updateMessage = async (messageId: string) => {
    ...
    try {
      if (isConnected) {
        ...
        // Fallback to HTTP
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/deal_room_conversations/${conversationId}/messages/${messageId}`,
          {
            method: 'PUT',
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        if (response.ok) {
          const data = await response.json();
          setMessages((prev) =>
            prev.map((msg) => (msg.id === messageId ? data.message_data : msg)),
          );
        }
      }
      ...
      showAlertMessage('Error', 'Failed to update message', 'error');
    }
  };
  ...
  const deleteMessage = async (messageId: string) => {
    ...
    // Store the message ID and show confirmation alert
  ...
  const confirmDeleteMessage = async () => {
    ...
    try {
      if (isConnected) {
        deleteWebSocketMessage(messageToDelete);
      } else {
        // Fallback to HTTP
        await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/deal_room_conversations/${conversationId}/messages/${messageToDelete}`,
          {
            method: 'DELETE',
            headers: {
              Authorization: `Bearer ${token}`,
      ...
      showAlertMessage('Error', 'Failed to delete message', 'error');
      setShowConfirmDelete(false);
      setMessageToDelete(null);
    }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showAlertMessage('Error', 'Failed to upload file', 'error');
    } finally {
      setIsSending(false);
