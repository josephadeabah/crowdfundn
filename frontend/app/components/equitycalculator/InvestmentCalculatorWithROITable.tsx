...
export default function InvestmentCalculatorColorfulTable({
  ...
  const [totalSharesExisting, setTotalSharesExisting] = useState<number>(10_000_000);
  const [dilutionFactor, setDilutionFactor] = useState<number>(0.7);

  // Derived calculations
  const postMoneyValuation = preMoneyValuation + (preMoneyValuation * (equityOffered / 100));
  const sharePrice = ((preMoneyValuation * (equityOffered / 100)) / (postMoneyValuation - preMoneyValuation)) * totalSharesExisting;
  const sharesPurchased = investment / sharePrice;
  const investorOwnership = sharesPurchased / (totalSharesExisting + sharesPurchased);

  // ROI calculation with dilution
  const calculateROI = (futureValuation: number) => {
    const futureValue = futureValuation * 1_000_000 * investorOwnership * dilutionFactor;
    return {
      futureValue,
      roiMultiple: investment > 0 ? futureValue / investment : 0,
      futureSharePrice: (futureValuation * 1_000_000) / (totalSharesExisting * (1 + (1 - dilutionFactor))),
    };
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-2xl space-y-6">
      <h1 className="text-3xl font-bold text-center">Startup Investment Calculator</h1>
      
      {/* Live ROI Preview */}
      {investment > 0 && (
        <div className="p-4 bg-blue-50 text-blue-800 rounded-xl text-center text-lg font-semibold shadow-sm">
          ✨ At ${preMoneyValuation / 1_000_000}M valuation, your ${investment} buys{' '}
          <strong>{sharesPurchased.toLocaleString(undefined, { maximumFractionDigits: 0 })} shares</strong>{' '}
          (<strong>{currentSummary.ownershipPercentage.toFixed(2)}%</strong> ownership)
        </div>
      )}
        ...
        <div className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) => setPreMoneyValuation((parseFloat(e.target.value) || 0) * 1_000_000)}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full p-3 border rounded-xl"
              value={equityOffered}
              onChange={(e) => setEquityOffered(Math.min(100, Math.max(0, parseFloat(e.target.value) || 0)))}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) => setTotalSharesExisting(Math.max(1, parseFloat(e.target.value) || 1))}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) => setDilutionFactor(1 - (Math.min(100, Math.max(0, parseFloat(e.target.value) || 0)) / 100))}
            />
          </div>
        </div>
        ...
        <div className="p-4 bg-gray-50 rounded-xl space-y-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span>{currentSummary.sharesPurchased.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
            </div>
            <div className="flex justify-between">
              <span>Ownership %:</span>
            ...
            <div className="flex justify-between">
              ...
              <span>${(currentSummary.postMoneyValuation / 1_000_000).toFixed(2)}M</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8">
        <h2 className="text-xl font-bold mb-4">📈 Potential Return Scenarios</h2>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {futureValuations.map((valuation) => {
                const { futureValue, roiMultiple, futureSharePrice } = calculateROI(valuation);
                const rowClass = roiMultiple > 10 ? 'bg-green-50' : roiMultiple > 3 ? 'bg-blue-50' : '';
                ...
                return (
                  <tr key={valuation} className={`border-t ${rowClass}`}>
                    ...
                    <td className="p-3">${futureValue.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
                    <td className="p-3 font-medium">{roiMultiple.toFixed(2)}x</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <div className="mt-6 p-4 bg-red-50 text-red-700 text-sm rounded-lg">
        <p><strong>⚠️ Important Disclaimer:</strong> This calculator provides illustrative projections only. Actual returns will vary based on:</p>
        ...
        <p className="mt-2">Startup investing carries high risk of total loss. Consult a financial advisor before making investment decisions.</p>
      </div>
    </div>
  );
}