...
import { Eye, CheckCircle, Clock, AlertTriangle, TrendingUp, Users, FileText, DollarSign, XCircle } from "lucide-react";
...
export function AdminCampaignReview() {
  ...
  const pendingCampaigns = equityCampaigns?.filter(
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">{equityCampaigns?.length || 0}</p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-yellow-600">{pendingCampaigns.length}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-green-600">
                  {equityCampaigns?.filter(c => c.equity_status === "live").length || 0}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">
                  ${equityCampaigns?.reduce((sum, campaign) => sum + parseFloat(campaign.transferred_amount || "0"), 0).toLocaleString() || "0"}
                </p>
              </div>
          ...
          <div className="space-y-4">
            {pendingCampaigns.map((campaign) => {
              const progress = campaign.goal_amount 
                ? (parseFloat(campaign.transferred_amount || "0") / parseFloat(campaign.goal_amount)) * 100 
                : 0;
                ...
                <div key={campaign.id} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">Created by {campaign.creator?.name || "Unknown"}</p>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Badge className={getStatusColor(campaign.equity_status)}>
                        {getStatusIcon(campaign.equity_status)}
                        <span className="ml-1 capitalize">{campaign.equity_status.replace('_', ' ')}</span>
                      </Badge>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-gray-600">
                        {campaign.documents?.filter(d => d.status === "approved").length || 0}/{campaign.documents?.length || 0} Documents
                      </span>
                    </div>
                    <div className="flex items-center space-x-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm text-gray-600">
                        ${parseFloat(campaign.transferred_amount || "0").toLocaleString()} / ${parseFloat(campaign.goal_amount || "0").toLocaleString()}
                      </span>
                    </div>
      ...
      <AlertPopup
        ...
        message={
          <div className="space-y-4">
            <p>Are you sure you want to reject {selectedCampaign?.title || "this campaign"}?</p>
            <div>
