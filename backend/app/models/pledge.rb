...
class Pledge < ApplicationRecord
  ...
  # Store shipping data with proper accessors
  store_accessor :shipping_data, :first_name, :last_name, :shipping_address, :entity_type

  # Store selected rewards with proper accessors  
  store_accessor :selected_rewards, :title, :description, :amount, :image

  # Custom JSON serialization
  def as_json(options = {})
    super(options.merge(
      only: %i[id donation_id equity_investment_id reward_id campaign_id user_id amount status shipping_status created_at],
      methods: [:shipping_first_name, :shipping_last_name, :shipping_address, :entity_type, :investment_type]
    )).merge(
  ...
  # Helper methods for shipping data
