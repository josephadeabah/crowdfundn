...
class DonationAbandonmentEmailService
  def self.send_abandonment_email(donation:, recipient_email:, recipient_name:, attempt_count:, gateway_response:)
    return false unless donation
    campaign = donation.campaign
    amount = donation.amount.round(2) if donation.amount
    currency_symbol = campaign&.currency_symbol || '₵'
    campaign_url = campaign ? Rails.application.routes.url_helpers.campaign_url(campaign, host: 'bantuhive.com') : 'https://bantuhive.com'

    subject = "Complete your donation to #{campaign&.title || 'the campaign'}"
    
    html_content = build_html_content(campaign, campaign_url, recipient_name, currency_symbol, amount, attempt_count, gateway_response, donation)
    text_content = build_text_content(campaign, campaign_url, recipient_name, currency_symbol, amount, attempt_count, gateway_response, donation)
  ...
  def self.build_html_content(campaign, campaign_url, recipient_name, currency_symbol, amount, attempt_count, gateway_response, donation)
    <<~HTML
      <!DOCTYPE html>
      <html>
        <head>
          ...
          <style>
            ...
            .header { background-color: #f39c12; padding: 30px 20px; text-align: center; color: white; }
            .header h1 { margin: 0; font-size: 24px; font-weight: 600; }
            .content { padding: 30px; }
            .greeting { font-size: 18px; margin-bottom: 20px; }
            .details { background-color: #f8f9fa; border-radius: 6px; padding: 20px; margin: 20px 0; border-left: 4px solid #f39c12; }
            ...
            .cta-button { display: inline-block; background-color: #27ae60; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; margin: 15px 0; }
          </style>
        </head>
        <body>
          <div class="email-container">
            ...
            <div class="content">
              ...
              <p>We noticed you started a donation but didn't complete it. Your support can make a real difference!</p>
              ...
              <div class="details">
                ...
                <div class="detail-row">
                  ...
                  <span class="detail-value">#{attempt_count}</span>
                </div>
                <div class="detail-row">
                  <span class="detail-label">Status:</span>
                  <span class="detail-value">Incomplete</span>
                </div>
              </div>

              <p>Don't miss the opportunity to support this important cause.</p>
              
              #{"<p><a href='#{campaign_url}' class='cta-button'>Complete Your Donation</a></p>" if campaign}
  ...
  def self.build_text_content(campaign, campaign_url, recipient_name, currency_symbol, amount, attempt_count, gateway_response, donation)
    <<~TEXT
      Hello #{recipient_name},

      We noticed you started a donation but didn't complete it. Your support can make a real difference!
      ...
      Don't miss the opportunity to support this important cause.

      #{campaign ? "Complete your donation: #{campaign_url}" : ""}
  ...
  def self.send_email(recipient_email, recipient_name, subject, html_content, text_content)
    begin
      ...
      Rails.logger.info "Successfully sent donation abandonment email to #{recipient_email}"
      response
    rescue => e
      Rails.logger.error "Failed to send donation abandonment email: #{e.message}"
      false
    end
  end
end