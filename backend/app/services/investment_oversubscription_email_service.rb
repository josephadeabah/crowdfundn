...
class InvestmentOversubscriptionEmailService
  ...
  def self.send_oversubscription_email(investment:, recipient_email:, recipient_name:, metadata: {})
    ...
    # Use simpler formatting to avoid helper issues
    shares = investment.shares.to_f.round(4)
    percentage = investment.percentage.to_f.round(4)
    amount = "#{campaign.currency_symbol}#{investment.amount.to_f.round(2)}"
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .investment-details { background-color: #fef5f5; border-radius: 6px; padding: 20px; margin: 20px 0; border-left: 4px solid #e74c3c; }
            .detail-row { display: flex; margin-bottom: 10px; }
            .detail-label { font-weight: 600; width: 180px; color: #555; }
            .detail-value { flex: 1; }
            .refund-info { background-color: #e8f5e8; border-radius: 6px; padding: 20px; margin: 20px 0; border-left: 4px solid #27ae60; }
            ...
            .cta-button { display: inline-block; padding: 12px 24px; background-color: #3498db; color: white; text-decoration: none; border-radius: 4px; font-weight: 600; }
          </style>
        </head>
        <body>
          <div class="email-container">
            <div class="header"><h1>Investment Update</h1></div>
            <div class="content">
              ...
              <div class="investment-details">
                ...
                <div class="detail-row"><span class="detail-label">Investment Amount:</span><span class="detail-value">#{amount}</span></div>
                <div class="detail-row"><span class="detail-label">Shares Requested:</span><span class="detail-value">#{shares} shares</span></div>
                <div class="detail-row"><span class="detail-label">Ownership Percentage:</span><span class="detail-value">#{percentage}%</span></div>
                <div class="detail-row"><span class="detail-label">Investment Date:</span><span class="detail-value">#{investment_date}</span></div>
              </div>

              <div class="refund-info">
                <h3 style="margin-top: 0; color: #27ae60;">Refund Information</h3>
                <p>A full refund of <strong>#{amount}</strong> will be processed to your original payment method within <strong>5-7 business days</strong>.</p>
                <p>Your refund reference number is: <strong>#{refund_reference}</strong></p>
              </div>

              <p>The campaign reached its maximum equity offering before your investment could be processed. We apologize for any inconvenience this may cause.</p>
              ...
              <p>If you have any questions about this process or would like assistance finding alternative investment opportunities, please contact our support team.</p>
              <p>We appreciate your understanding and hope to serve you better in the future.</p>
              <p>Sincerely,<br><strong>The Bantuhive Investments Team</strong></p>
            </div>
            <div class="footer">
  ...
  private_class_method def self.send_email(recipient_email, recipient_name, subject, html_content, text_content)
    begin
      send_smtp_email = SibApiV3Sdk::SendSmtpEmail.new(
        ...
        sender: { name: 'Bantuhive Investments', email: 'help@bantuhive.com' },
        headers: { 'X-Mailin-custom' => 'investment_oversubscription' }
      )
