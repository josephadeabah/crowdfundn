...
class DonationRefundEmailService
  def self.send_refund_email(donation:, recipient_email:, recipient_name:, reason:)
    ...
    subject = "Your donation refund has been processed"
    
    html_content = build_html_content(campaign, campaign_url, recipient_name, currency_symbol, amount, reason, donation)
    text_content = build_text_content(campaign, campaign_url, recipient_name, currency_symbol, amount, reason, donation)
  ...
  def self.build_html_content(campaign, campaign_url, recipient_name, currency_symbol, amount, reason, donation)
    <<~HTML
      <!DOCTYPE html>
      <html>
        <head>
          ...
          <title>Donation Refund Processed</title>
          <style>
            ...
            .details { background-color: #f8f9fa; border-radius: 6px; padding: 20px; margin: 20px 0; border-left: 4px solid #3498db; }
        ...
        <body>
          <div class="email-container">
            <div class="header">
              <h1>Donation Refund Processed</h1>
            </div>
            ...
            <div class="content">
              ...
              <div class="details">
                ...
                #{"<div class='detail-row'><span class='detail-label'>Refund Amount:</span><span class='detail-value'>#{currency_symbol}#{amount}</span></div>" if amount}
              ...
              <p>If you'd like to make another donation, we'd be grateful for your continued support.</p>
  ...
  def self.build_text_content(campaign, campaign_url, recipient_name, currency_symbol, amount, reason, donation)
    <<~TEXT
      ...
      #{amount ? "Refund Amount: #{currency_symbol}#{amount}" : ""}
      Reason: #{reason}
      Reference: #{donation.transaction_reference}
      Refund Date: #{Time.current.strftime('%B %d, %Y')}

      The refund should reflect in your account within 3-5 business days, depending on your bank's processing time.

      If you'd like to make another donation, we'd be grateful for your continued support.

      #{campaign ? "Make another donation: #{campaign_url}" : ""}

      If you have any questions about this refund, please contact our support team at help@bantuhive.com.
  ...
  def self.send_email(recipient_email, recipient_name, subject, html_content, text_content)
    begin
      ...
      Rails.logger.info "Successfully sent donation refund email to #{recipient_email}"
      response
    rescue => e
      Rails.logger.error "Failed to send donation refund email: #{e.message}"
      false
    end
  end
end