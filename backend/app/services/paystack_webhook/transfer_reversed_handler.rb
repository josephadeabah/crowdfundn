  ...
  class TransferReversedHandler
    ...
    def call
      ...
      if EventProcessed.exists?(event_id: transfer_reference)
        Rails.logger.info "Transfer reversed event already processed: #{transfer_reference}"
        ...
        ActiveRecord::Base.transaction do
          ...
          if subaccount
            subaccount.update!(
              ...
              recipient_code: @data.dig(:recipient, :recipient_code),
              amount: (@data[:amount]).to_f / 100, # Use gross amount
              transfer_code: @data[:transfer_code],
              reference: @data[:reference],
              account_number: @data.dig(:recipient, :details, :account_number),
      ...
      Rails.logger.error "Error processing transfer reversal for reference #{transfer_reference}: #{e.message}"
      raise e
    ensure
